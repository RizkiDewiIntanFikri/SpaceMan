...
// We need to import the stores to call their actions when we receive an event
...
export const connectSocket = (token) => {
    ...
    socket = io('http://localhost:3030'); // Use your backend's URL

    // --- Set up event listeners ---

    // This event fires when the connection is first established
    socket.on('connect', () => {
    ...
    socket.on('leaderboard-update', (newLeaderboardData) => {
        ...
        // Call the action in our marketStore
        useMarketStore.getState().updateLeaderboardFromSocket(newLeaderboardData);
    });
